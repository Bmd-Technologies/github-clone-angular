...
export class AppComponent {
  ...
  async getUserProfile(){
    ...
    let url_user= `https://api.github.com/users/${this.username}`
    ...
    if(response.status === 200){
      ...
      if(this.userProfile.public_repos > 0){
        ...
        let url_repo = `https://api.github.com/users/${this.username}/repos`;
  ...
  async getRepoOfPage(value:number){
    ...
    let url_repo = `https://api.github.com/users/${this.username}/repos?page=${this.currentpage}`;
        const repo_response = await fetch(url_repo);
        const repo_json = await repo_response.json();
        this.isReopLoading=false;
        if(repo_response.status === 200){
